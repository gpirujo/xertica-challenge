...
def main():
    ...
    from_date = args.from_date or (now - timedelta(days=30)).date().isoformat()
    from_ts = f"{from_date}T00:00:00Z"
    to_ts = f"{to_date}T23:59:59Z"

    def _fetch(view, measure, aggregation, filter_name, filter_value, dimensions=None):
        q = {
            "view": view,
    ...
    report["total_traces"] = _fetch_count()
